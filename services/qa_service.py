...
from services.kb_service import (
    save_chat, save_evaluation_record, get_accessible_ragflow_doc_ids,
)
...
def ask_stream(kb_id: str, question: str, top_k: int = 5, session_id: str = None,
    ...
    if session_id:
        ...
        history = get_session_history(kb_id, session_id, user=current_user)
        if history:
            parts = []
    ...
    # 2. 两级过滤：MySQL 查权限 → RAGFlow 限定文档检索
    allowed_ids = get_accessible_ragflow_doc_ids(current_user, kb_id) if current_user else None

    if not dataset_id and not rag.available:
        search_results = rag._local_search(question, top_k, allowed_ids)
    else:
        search_results = rag.search(dataset_id, question, top_k, allowed_ids)
    ...
    save_chat(kb_id, question, answer, sources, session_id, user_id=user_id)

    # 6. 保存评估记录（只有 admin 的问答进入评估，普通用户受权限限制检索不全）
...
def ask_sync(kb_id: str, question: str, top_k: int = 5,
    ...
    # 保存记录（所有用户保存聊天记录，只有 admin 进入评估）
    user_id = current_user.id if current_user else ""
    save_chat(kb_id, question, answer, sources, user_id=user_id)
    if current_user and getattr(current_user, "role", "") == "admin":
        save_evaluation_record(
